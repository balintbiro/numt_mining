...
def grid_search(n_neighbors,min_dist,title):
    ...
    plt.savefig(f'../results/UMAPs/{title}_{n_neighbors}_nn_{min_dist}_md.png',dpi=450)
...
for feature1 in X_labeled.columns:
    for feature2 in X_labeled.columns:
        try:
            if feature1!=feature2:
                X_temp=X_labeled[[feature1,feature2,'genus_label','family_label','order_label','label']].dropna()
                X=X_temp[[feature1,feature2]]
                grid_search(100,0.99,f'{feature1}_{feature2}')
        except:
            pass
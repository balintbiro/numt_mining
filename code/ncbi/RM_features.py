...
def repeatmasker(organism_name,sequence_type):
	...
	#call RepeatMasker
	call(f"""RepeatMasker -species {organism_name.replace('_',' ')} ../data/RM_files/RM_input.fa""",shell=True)
	#transform RepeatMasker.out file into a tab separated file for better handling
	call("""cat ../data/RM_files/RM_input.fa.out | tr -s ' ' | sed 's/^ *//g' | tr ' ' '\t' > ../data/RM_files/RM_input.fa.out.tab""",shell=True)
	#add columns https://www.repeatmasker.org/webrepeatmaskerhelp.html How to read the results
	repeats=pd.read_csv('../data/RM_files/RM_input.fa.out.tab',sep='\t',skiprows=2,header=None)
	repeats.columns=[
	'SW_score','div_perc','del_perc','ins_perc','query_name','piq_begin','piq_end','piq_left','matching','repeat',
	'repeat_class','pir_begin','pir_end','pin_left','ID','higher_score_match'
	...
	repeats.to_csv('../data/RM_files/repeats.csv')

#make the function work
repeatmasker('acinonyx_jubatus','upstream_5kb')